...
<body>
	...
	<div class="Competence_add_style clearfix">
		<form action="{{route('admin.privilege.update',['id'=>$priInfo->id])}}" method="post">
		{{csrf_field()}}
		{{ method_field('PATCH')}}
			<div class="left_Competence_add">
				...
				<div class="Competence_add">
					<div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 权限名称 </label>
						<div class="col-sm-9"><input type="text" id="form-field-1" placeholder="" name="role_name" class="col-xs-10 col-sm-5" value="{{$priInfo->role_name}}"></div>
					</div>
					<div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 权限描述 </label>
						<div class="col-sm-9"><textarea name="describe" class="form-control" id="form_textarea" placeholder="" onkeyup="checkLength(this);">{{$priInfo->describe}}</textarea><span
					...
					<div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 用户选择 </label>
						<div class="col-sm-9">
							@foreach($admin as $v)
							<label class="middle">
								@if(in_array($v->id,$admin_id))
									<input class="ace" checked="checked" type="checkbox" id="id-disable-check" name="admin[]" value="{{$v->id}}">
								@else
									<input class="ace" type="checkbox" id="id-disable-check" name="admin[]" value="{{$v->id}}">
								@endif
			...
			<div class="Assign_style">
				...
				<div class="Select_Competence">
					...
					<dl class="permission-list">
						...
						<dt><label class="middle"><input name="pri_admin[]" checked="checked" value="{{$v['id']}}" class="ace" type="checkbox" id="id-disable-check"><span
						...
						<dd>
							...
							<dl class="cl permission-list2">
								@if(in_array($v2['id'],$pri_id))
								<dt><label class="middle"><input checked="checked"  type="checkbox" value="{{$v2['id']}}" class="ace" name="pri_admin[]" id="id-disable-check"><span
								...
								<dd>
									@foreach($v2['child'] as $v3)
										...
										@if(in_array($v3['id'],$pri_id))
										<label class="middle"><input checked="checked" type="checkbox" value="{{$v3['id']}}" class="ace" name="pri_admin[]" id="user-Character-0-0-0"><span
