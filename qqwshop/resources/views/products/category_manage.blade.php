  ...
  <div id="brand_style">
    <div class="search_style">
      ...
      <ul class="search_content clearfix">
       ...
       <li><select name="" class="text_add"><option  value="1">国内分类</option><option value="2">国外分类</option></select></li>
       <li style="width:90px;"><button type="button" class="btn_search"><i class="icon-search"></i>查询</button></li>
      </ul>
    </div>
     <div class="border clearfix">
       <span class="l_f">
        <a href="{{route('brand.create')}}"  title="添加分类" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加分类</a>
	...
	<tbody>
        @foreach($goods_categorie as $v)
		<tr>
          <td width="25px"><label><input type="checkbox" class="ace" ><span class="lbl"></span></label></td>
          <td width="80px">{{$v->id}}</td>
          <td class="td_left"><a href="javascript:ovid()" name="" style="cursor:pointer" class="text-primary brond_name" title="{{$v->cate_name}}">{{ str_repeat('--',$v->level).$v->cate_name}}</a></td>
          <td class="td-manage">
...
laydate({
 ...
 <script type="text/javascript">
        require.config({
            paths: {
                echarts: '/admin_style/assets/dist'
            }
        });
        require(
            [
